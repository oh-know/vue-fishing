<template lang="html">
  <div class="fish allbg" >
    ...
    <div class="select">
      ...
      <div class="zhineng sec" :class="{isBorder: Tab ==='brief2'}" @click.stop.prevent="sort('brief2')">
        智能排序<span class="sanB" v-show="bottom2"></span><span class="sanA" v-show="top2"></span>
      </div>
      <div class="type sec" :class="{isBorder: Tab ==='brief3'}" @click.stop.prevent="type('brief3')">
        类型<span class="sanB" v-show="bottom3"></span><span class="sanA" v-show="top3"></span>
      </div>
    </div>
    <div class="mask" v-show="s" @click.stop.prevent="hidden" @touchmove.prevent></div>
    <div class="msg" v-show="a">
      <span @click.stop.prevent="selectCity" class="qy" ref="qy"></span>
      <img src="static/images/shuaxin.png" @click="update" class="update">
    </div>
    <transition name="slide-left">
      <!-- <div class="fff"> -->
        <!-- <div class="city_select" v-show="d">
          <div class="left" v-for="(item, index) in citys" key="item">
            <div class="city_name" :class="{lpl: Tob=== index}" @click="selectArea(index)">
              {{item}}
            </div>
          </div>
        </div>
        <div class="right" v-show="another">
          <div class="city_name" v-for="(item, index) in 6" >
            <span :class="{lck: Tabber=== index}" @click="selectCrea(index)">东湖区</span>
            <div :class="{gou: Tabber=== index}" @click="selectCrea(index)"></div>
          </div>
        </div> -->
        <!-- <div class="membership1"> -->
     <section class="showChose" v-show="d">
      <section class="address">
        <!-- <ul>
          <li class="addList" v-for="(v,k) in info" @click="getProvinceId(v.id, v.name, k)"  :class="v.selected ? 'active' : ''">{{v.name}}</li>
        </ul> -->
        <ul>
          <li class="addList" v-for="(v,k) in showCityList" @click="getCityId(v.id, v.name, k)"  :class="v.selected ? 'active' : ''">{{v.name}}</li>
        </ul>
        <ul>
          <li class="addList" v-for="(v,k) in showDistrictList" @click="getDistrictId(v.id, v.name, k)" :class="v.selected ? 'active' : ''">{{v.name}}</li>
        </ul>
      </section>
    </section>
  <!-- </div> -->
    ...
    <transition name="slide-bottom">
      <div class="msg1" v-show="b" @click="hidden">
        <div class="new optionss" @click="getNewData" @touchmove.prevent>
          最新鱼汛
        </div>
        <div class="lately optionss" @click="getNearData" @touchmove.prevent>
          离我最近
        </div>
        <div class="highest optionss" @click="getHotData" @touchmove.prevent>
          人气最高
        </div>
      </div>
    </transition>
    <transition name="slide-bottom">
      <div class="msg2" v-show="c" @click="hidden">
        <div class="fish_type" @click="changeType(false)">
          不限
        </div>
        <div class="fish_type" v-for="item in types" :key="item.id"
    ...
    <div class="a">
      ...
      <mu-list>
        ...
        <fishfield-item :lists="lists" :cateid="cateid"></fishfield-item>
      </mu-list>
...
export default {
    data() {
        return {
            ...
            types: [],
            // fishfield: '',
            citys: ['南昌', '九江', '上饶', '赣州', '宜春', '新余', '景德镇', '萍乡'],
            a: true,
            b: false,
            c: false,
            d: false,
    ...
    computed: {
        showChose() {
            return this.$store.state.membership.showChose
        },
        localCity() {
            return this.$store.state.selectCity.localCity
        },
        ydCity() {
            return this.$store.state.membership.ydCity
        },
        ydDistrict() {
            return this.$store.state.membership.ydDistrict
        },
        tag() {
            return this.$store.state.fish.tag
        },
        fishfield() {
            return this.$store.state.fish.fishfield
        }
    },
    mounted() {
        // 设置参照物与比较物
        ...
        this.getData()
        if (this.ydCity == '' && this.ydDistrict == '') {
            console.log('hj')
            this.$refs.qy.innerText = '请选择区域'
        } else {
            this.$refs.qy.innerText = this.ydCity + this.ydDistrict
        }
        ...
        var index = 0
        for (var i = 0; i < this.info.length; i++) {
            if (this.info[i].name == '江西') {
                index = i
            }
        }
        var index1 = 0
        for (var j = 0; j < this.info[index].city.length; j++) {
            if (this.info[index].city[j].name == this.localCity + '市') {
                this.city = this.info[index].city[j].id
                index1 = j
            }
        }
        // this.city = 125;
        this.City = this.localCity + '市'
        this.showProvince = false;
        this.showCity = false;
    ...
    methods: {
        ...
        closeAdd: function () {
            this.$store.commit('showChose', false)
        },
        ...
        getDistrictId: function (code, input, index) {
            ...
            // 选取市区选项之后关闭弹层
            this.$store.commit('showChose', false)
            this.$store.commit('ydAddres', { city: this.City, district: this.District })
            this.d = false
            this.getData()
            if (this.ydCity == '' && this.ydDistrict == '') {
              this.$refs.qy.innerText = '请选择区域'
        ...
        update() {
          this.$store.commit('ydAddres', { city: '', district: '' })
          this.$store.commit('selectFishfield', '')
            ...
            this.axios.post('http://yulejia.pnnnc.com/api/getfishing', params)
                .then(res => {
                    if (res.data.code == '0') {
                        ...
                        this.$router.push('/addfishfield')
                    }
                })
        },
        loadMore() {
            const totalNum = this.lists.length
            ...
            if (this.page < pageNum) {
                ...
                setTimeout(() => {
                    ...
                    param.append('city', '南昌市')
                    param.append('is_tui', false)
                    param.append('tag', this.tag)
                    param.append('district', this.ydDistrict)
                    param.append('get_cate', true)
                    param.append('cate_id', this.fishfield)
        ...
        domain(type) {
            ...
            if (this.d) {
                ...
                this.bottom1 = true
                this.top1 = false
                this.bottom2 = this.bottom3 = true
            } else {
                this.another = true
                this.bottom1 = false
                this.top1 = true
        ...
        sort(type) {
            ...
            if (this.b) {
                ...
                this.top2 = true
                this.b = true
                this.bottom1 = this.bottom3 = true
                this.top1 = this.top3 = false
            }
        },
        type(type) {
            this.Tab = type
            ...
            if (this.c) {
                ...
                this.top3 = true
                this.c = true
                this.bottom1 = this.bottom2 = true
                this.top1 = this.top2 = false
            }
        },
        selectCity() {
            this.a = true
        ...
        getData() {
            ...
            param.append('get_cate', true)
            param.append('cate_id', this.fishfield)
            this.axios.post(apiurl.apiurl+'/api/getfishing', param)
                .then(res => {
                    if (res.data.code === 1) {
                        let datas = res.data.data.data
                        let head = 'http://yulejia.pnnnc.com/uploads/images/'
        ...
        changeType(id) {
            if (id) {
                // this.fishfield = id
                this.$store.commit('selectFishfield', id)
                this.getData()
            } else {
                this.$store.commit('selectFishfield', '')
                this.getData()
...
.sanA {
    ...
    border-bottom: .15rem solid #F39800;
    margin-left: .2rem;
    margin-top: .23rem;
}
